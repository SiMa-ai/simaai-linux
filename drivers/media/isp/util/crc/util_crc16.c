...
#define CRC_NEXT( crc, byte, crc_lut ) ( crc_lut[( ( byte ) ^ ( ( crc ) >> 8 ) ) & 0xff] ^ ( ( crc ) << 8 ) )

#if CRC16_POLY_CCITT
static const uint16_t ccitt_lut[0x100] = {
...
/**
 * @brief      Calcualtes CRC CCITT of a memory block size sz
 *
 ...
 */
uint16_t
acrc_ccitt_memory( const uint16_t seed, const unsigned char *data, const size_t sz )
{
    uint16_t crc = seed;
    size_t count = sz;
    while ( count-- ) {
        crc = CRC_NEXT( crc, *data++, ccitt_lut );
    }
    return crc;
}

/**
 * @brief      Calculates CRC CCITT of a single byte, useful for bytewise
 *             calculations
 *
 * @param[in]  seed  The seed
 ...
 *
 */
uint16_t acrc_ccitt_byte( const uint16_t seed, const unsigned char byte )
{
    return CRC_NEXT( seed, byte, ccitt_lut );
}

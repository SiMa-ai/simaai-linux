...
static __inline uint32_t sbus_update_address( acamera_sbus_ptr_t p_bus, uint32_t addr )
{
...
uint16_t mem_read_u16( void *p_data )
{
    if ( ( (size_t)p_data ) & 1 ) {
...
uint32_t mem_read_u32( void *p_data )
{
...
void mem_write_u16( void *p_buf, uint16_t data )
{
    if ( ( (size_t)p_buf ) & 1 ) {
...
void mem_write_u32( void *p_buf, uint32_t data )
{
...
uint8_t acamera_sbus_read_u8( acamera_sbus_t *p_bus, uint32_t addr )
{
...
uint16_t acamera_sbus_read_u16( acamera_sbus_t *p_bus, uint32_t addr )
{
...
uint32_t acamera_sbus_read_u32( acamera_sbus_t *p_bus, uint32_t addr )
{
...
void acamera_sbus_read_data_u8( acamera_sbus_t *p_bus, uint32_t addr, uint8_t *p_data, int n_count )
{
    int i;
    for ( i = 0; i < n_count; ++i ) {
...
void acamera_sbus_read_data_u16( acamera_sbus_t *p_bus, uint32_t addr, uint16_t *p_data, int n_count )
{
    ...
    if ( ( (size_t)p_data ) & 1 ) {
        ...
        for ( i = 0; i < n_count; ++i, ++p_data ) {
            mem_write_u16( p_data, acamera_sbus_read_u16( p_bus, addr ) );
            addr += addr_step;
        }
    } else {
        for ( i = 0; i < n_count; ++i ) {
...
void acamera_sbus_read_data_u32( acamera_sbus_t *p_bus, uint32_t addr, uint32_t *p_data, int n_count )
{
    ...
    if ( ( (size_t)p_data ) & 3 ) {
        // unaligned read
        for ( i = 0; i < n_count; ++i, ++p_data ) {
            mem_write_u32( p_data, acamera_sbus_read_u32( p_bus, addr ) );
            addr += addr_step;
        }
    } else {
        for ( i = 0; i < n_count; ++i ) {
...
void acamera_sbus_write_u8( acamera_sbus_t *p_bus, uint32_t addr, uint8_t sample )
{
    if ( p_bus->mask & SBUS_MASK_SAMPLE_8BITS ) {
...
void acamera_sbus_write_u16( acamera_sbus_t *p_bus, uint32_t addr, uint16_t sample )
{
    if ( p_bus->mask & SBUS_MASK_SAMPLE_16BITS ) {
        if ( p_bus->mask & SBUS_MASK_SAMPLE_SWAP_BYTES ) {
...
void acamera_sbus_write_u32( acamera_sbus_t *p_bus, uint32_t addr, uint32_t sample )
{
    if ( p_bus->mask & SBUS_MASK_SAMPLE_32BITS ) {
        if ( p_bus->mask & SBUS_MASK_SAMPLE_SWAP_BYTES ) {
...
void acamera_sbus_write_data_u8( acamera_sbus_t *p_bus, uint32_t addr, uint8_t *p_data, int n_count )
{
    int i;
    for ( i = 0; i < n_count; ++i ) {
...
void acamera_sbus_write_data_u16( acamera_sbus_t *p_bus, uint32_t addr, uint16_t *p_data, int n_count )
{
    const int addr_step = SBUS_ADDRESS_16BIT_INCREMENT( p_bus );
    int i;
    if ( ( (size_t)p_data ) & 1 ) {
        // Unaligned write
        for ( i = 0; i < n_count; ++i, ++p_data ) {
            acamera_sbus_write_u16( p_bus, addr, mem_read_u16( p_data ) );
            addr += addr_step;
        }
    } else {
        for ( i = 0; i < n_count; ++i ) {
...
void acamera_sbus_write_data_u32( acamera_sbus_t *p_bus, uint32_t addr, uint32_t *p_data, int n_count )
{
    const int addr_step = SBUS_ADDRESS_32BIT_INCREMENT( p_bus );
    int i;
    if ( ( (size_t)p_data ) & 3 ) {
        // Unaligned read
        for ( i = 0; i < n_count; ++i, ++p_data ) {
            acamera_sbus_write_u32( p_bus, addr, mem_read_u32( p_data ) );
            addr += addr_step;
        }
    } else {
        for ( i = 0; i < n_count; ++i ) {
...
void acamera_sbus_write_data( acamera_sbus_t *p_bus, uint32_t addr, void *p_data, int n_size )
{
...
void acamera_sbus_copy( acamera_sbus_t *p_bus_to, uint32_t addr_to, acamera_sbus_t *p_bus_from, uint32_t addr_from, int n_size )
{
    if ( n_size > 4 ) {
...
void acamera_sbus_deinit( acamera_sbus_t *p_bus, sbus_type_t interface_type )
{
    if ( p_bus != NULL ) {
        ...
        case sbus_i2c:
            ...
            break;
        case sbus_spi:
            acamera_sbus_spi_deinit( p_bus );
            break;
            /*case sbus_isp:
                                acamera_sbus_isp_deinit( p_bus );*/
            break;
        default:
            LOG( LOG_CRIT, "Invalid SBUS protocol." );
            break;

...
static void spi_io_write_sample( acamera_sbus_t *p_sbus, uint32_t addr, uint32_t sample, uint8_t sample_size )
{
    ...
    if ( sample_size == 4 ) {
        ...
        {
            data = sample;
            if ( ( mask & SBUS_MASK_ADDR_24BITS ) == SBUS_MASK_ADDR_16BITS ) {
                data <<= 16;
                data |= addr;
            } else if ( ( mask & SBUS_MASK_ADDR_24BITS ) == SBUS_MASK_ADDR_24BITS ) {
                data <<= 16;
                data |= addr;
                data <<= 8;
...
static uint32_t spi_io_read_sample( acamera_sbus_t *p_sbus, uint32_t addr, uint8_t sample_size )
{
    uint32_t mask = p_sbus->mask;
    uint32_t sel_mask = 1 << p_sbus->device;
    uint32_t data = 0x0;
    ...
    if ( sample_size == 4 ) {
        ...
        if ( ( mask & SBUS_MASK_ADDR_24BITS ) == SBUS_MASK_ADDR_24BITS ) {
            addr <<= 8;
...
void acamera_sbus_spi_init( acamera_sbus_ptr_t p_sbus )
{
    ...
    system_spi_init( p_sbus->bus );
}
void acamera_sbus_spi_deinit( acamera_sbus_t *p_bus )
{
    //system_spi_deinit( p_bus->bus );
}

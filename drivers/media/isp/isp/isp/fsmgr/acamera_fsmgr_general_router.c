...
static acamera_cmd_result general_raw_scaler_ratio( acamera_isp_ctx_ptr_t p_ictx, acamera_cmd_direction dir, const uint32_t *value, uint32_t *ret_value )
{
    ...
    raw_scaler_get_general_invert_ratio( &( p_fsmgr->raw_scaler_fsm ), ret_value );
#else
    *ret_value = 1048576; /* This value represent ratio of 1.0 in 4.20 format */
#endif

    return CMD_RESULT_OK;
}

static acamera_cmd_result general_ae_info( acamera_isp_ctx_ptr_t p_ictx, acamera_cmd_direction dir, const uint32_t *value, uint32_t *ret_value )
{
    if ( CMD_DIRECTION_SET == dir ) {
        LOG( LOG_NOTICE, "SET is not supported." );
        return CMD_RESULT_INVALID_PARAM;
...
static acamera_cmd_result general_cmos_current_exposure_log2( acamera_isp_ctx_ptr_t p_ictx, acamera_cmd_direction dir, const uint32_t *value, uint32_t *ret_value )
{
    if ( CMD_DIRECTION_SET == dir ) {
        LOG( LOG_NOTICE, "SET is not supported." );
        return CMD_RESULT_NOT_SUPPORTED;
...
static acamera_cmd_result general_cmos_exposure_ratio( acamera_isp_ctx_ptr_t p_ictx, acamera_cmd_direction dir, const uint32_t *value, uint32_t *ret_value )
{
    if ( CMD_DIRECTION_SET == dir ) {
        LOG( LOG_NOTICE, "SET is not supported." );
        return CMD_RESULT_NOT_SUPPORTED;
...
static acamera_cmd_result general_cmos_max_exposure_log2( acamera_isp_ctx_ptr_t p_ictx, acamera_cmd_direction dir, const uint32_t *value, uint32_t *ret_value )
{
    if ( CMD_DIRECTION_SET == dir ) {
        LOG( LOG_NOTICE, "SET is not supported." );
        return CMD_RESULT_NOT_SUPPORTED;
...
static acamera_cmd_result general_cmos_history_size_get( acamera_isp_ctx_ptr_t p_ictx, acamera_cmd_direction dir, const uint32_t *value, uint32_t *ret_value )
{
    if ( CMD_DIRECTION_SET == dir ) {
        LOG( LOG_NOTICE, "SET is not supported." );
        return CMD_RESULT_NOT_SUPPORTED;
...
static acamera_cmd_result general_exposure_target( acamera_isp_ctx_ptr_t p_ictx, acamera_cmd_direction dir, const uint32_t *value, uint32_t *ret_value )
{
...
static acamera_cmd_result general_frame_exposure_set( acamera_isp_ctx_ptr_t p_ictx, acamera_cmd_direction dir, const uint32_t *value, uint32_t *ret_value )
{
    if ( CMD_DIRECTION_SET == dir ) {
        LOG( LOG_NOTICE, "SET is not supported." );
        return CMD_RESULT_NOT_SUPPORTED;
...
static acamera_cmd_result general_cmos_total_gain( acamera_isp_ctx_ptr_t p_ictx, acamera_cmd_direction dir, const uint32_t *value, uint32_t *ret_value )
{
    if ( CMD_DIRECTION_SET == dir ) {
        LOG( LOG_NOTICE, "SET is not supported." );
        return CMD_RESULT_NOT_SUPPORTED;
...
static acamera_cmd_result general_cmos_analog_gain( acamera_isp_ctx_ptr_t p_ictx, acamera_cmd_direction dir, const uint32_t *value, uint32_t *ret_value )
{
    if ( CMD_DIRECTION_SET == dir ) {
        LOG( LOG_NOTICE, "SET is not supported." );
        return CMD_RESULT_NOT_SUPPORTED;
...
#if defined( ISP_HAS_USER2KERNEL_FSM )
    // user-driver takes the cmos sensor analog gain log2 from user2kernel sbuf
    cmos_info_t sbuf_cmos_info;
    user2kernel_get_sbuf_cmos_info( p_ictx->context_id, &sbuf_cmos_info );
    *( (int32_t *)ret_value ) = sbuf_cmos_info.again_log2;
    ...
    *( (int32_t *)ret_value ) = p_fsmgr->cmos_fsm.again_val_log2;
#else
#error "No general router handler for CMD_ID_CMOS_ANALOG_GAIN !"
#endif

    return CMD_RESULT_OK;
}

static acamera_cmd_result general_cmos_digital_gain( acamera_isp_ctx_ptr_t p_ictx, acamera_cmd_direction dir, const uint32_t *value, uint32_t *ret_value )
{
    if ( CMD_DIRECTION_SET == dir ) {
        LOG( LOG_NOTICE, "SET is not supported." );
        return CMD_RESULT_NOT_SUPPORTED;
...
#if defined( ISP_HAS_USER2KERNEL_FSM )
    // user-driver takes the cmos sensor digital gain log2 from user2kernel sbuf
    cmos_info_t sbuf_cmos_info;
    user2kernel_get_sbuf_cmos_info( p_ictx->context_id, &sbuf_cmos_info );
    *( (int32_t *)ret_value ) = sbuf_cmos_info.dgain_log2;

#elif defined( ISP_HAS_CMOS_FSM )
    acamera_fsmgr_t *p_fsmgr = &p_ictx->fsmgr;

    *( (int32_t *)ret_value ) = p_fsmgr->cmos_fsm.dgain_val_log2;
#else
#error "No general router handler for CMD_ID_CMOS_DIGITAL_GAIN !"
#endif

    return CMD_RESULT_OK;
}

static acamera_cmd_result general_ccm_info( acamera_isp_ctx_ptr_t p_ictx, acamera_cmd_direction dir, const uint32_t *value, uint32_t *ret_value )
{
    // parameter check
    if ( ( CMD_DIRECTION_GET == dir ) && ( NULL == ret_value ) ) {
        LOG( LOG_ERR, "Invalid param: ret_value is NULL." );
...
static acamera_cmd_result general_wb_info( acamera_isp_ctx_ptr_t p_ictx, acamera_cmd_direction dir, const uint32_t *value, uint32_t *ret_value )
{
    // parameter check
    if ( ( CMD_DIRECTION_GET == dir ) && ( NULL == ret_value ) ) {
        LOG( LOG_ERR, "Invalid param: ret_value is NULL." );
...
static acamera_cmd_result general_wb_frame_quantity( acamera_isp_ctx_ptr_t p_ictx, acamera_cmd_direction dir, const uint32_t *value, uint32_t *ret_value )
{
...
static acamera_cmd_result general_sensor_info( acamera_isp_ctx_ptr_t p_ictx, acamera_cmd_direction dir, const uint32_t *value, uint32_t *ret_value )
{
    // parameter check
    if ( ( CMD_DIRECTION_GET == dir ) && ( NULL == ret_value ) ) {
        LOG( LOG_ERR, "Invalid param: ret_value is NULL." );
...
static acamera_cmd_result general_sensor_preset_info( acamera_isp_ctx_ptr_t p_ictx, acamera_cmd_direction dir, const uint32_t *value, uint32_t *ret_value )
{
    // parameter check
    if ( ( CMD_DIRECTION_GET == dir ) && ( NULL == ret_value ) ) {
        LOG( LOG_ERR, "Invalid param: ret_value is NULL." );
    ...
    acamera_cmd_sensor_preset_info_t *p_sensor_preset_info = (acamera_cmd_sensor_preset_info_t *)ret_value;

    system_memset( p_sensor_preset_info, 0, sizeof( acamera_cmd_sensor_preset_info_t ) );

    if ( sensor_get_general_sensor_preset_info( &p_fsmgr->sensor_fsm, *value, p_sensor_preset_info ) ) {
        return CMD_RESULT_INVALID_PARAM;
    }
...
#else
#error "No general router handler for CMD_ID_SENSOR_PRESET_INFO !"
#endif

    return CMD_RESULT_OK;
}

static acamera_cmd_result general_sensor_alloc_analog_gain( acamera_isp_ctx_ptr_t p_ictx, acamera_cmd_direction dir, const uint32_t *value, uint32_t *ret_value )
{
    // right now only support GET
    if ( CMD_DIRECTION_SET == dir ) {
        LOG( LOG_ERR, "Invalid param: Set is not supported" );
...
#elif defined( ISP_HAS_USER2KERNEL_FSM )
    // Get sensor analog gain allocated by the kernel CMOS code
    return general_cmos_analog_gain( p_ictx, dir, value, ret_value );
#else
...
static acamera_cmd_result general_sensor_alloc_digital_gain( acamera_isp_ctx_ptr_t p_ictx, acamera_cmd_direction dir, const uint32_t *value, uint32_t *ret_value )
{
    // right now only support GET
    if ( CMD_DIRECTION_SET == dir ) {
        LOG( LOG_ERR, "Invalid param: Set is not supported" );
...
#elif defined( ISP_HAS_USER2KERNEL_FSM )
    // Get sensor digital gain allocated by the kernel CMOS code
    return general_cmos_digital_gain( p_ictx, dir, value, ret_value );
#else
...
static acamera_cmd_result general_sensor_update( acamera_isp_ctx_ptr_t p_ictx, acamera_cmd_direction dir, const uint32_t *value, uint32_t *ret_value )
{
    // right now only support SET
    if ( CMD_DIRECTION_GET == dir ) {
        LOG( LOG_ERR, "Invalid param: Get is not supported" );
...
static acamera_cmd_result general_sensor_alloc_integration_time( acamera_isp_ctx_ptr_t p_ictx, acamera_cmd_direction dir, const uint32_t *value, uint32_t *ret_value )
{
    // right now only support SET
    if ( CMD_DIRECTION_GET == dir ) {
        LOG( LOG_ERR, "Invalid param: Get is not supported" );
...
static acamera_cmd_result general_sensor_set_wdr_mode( acamera_isp_ctx_ptr_t p_ictx, acamera_cmd_direction dir, const uint32_t *value, uint32_t *ret_value )
{
    // right now only support SET
    if ( CMD_DIRECTION_GET == dir ) {
        LOG( LOG_ERR, "Invalid param: Get is not supported" );
...
static acamera_cmd_result general_sensor_alloc_channel_dgains( acamera_isp_ctx_ptr_t p_ictx, acamera_cmd_direction dir, const uint32_t *value, uint32_t *ret_value )
{
#if defined( ISP_HAS_SENSOR_FSM )
    // right now only support SET
    if ( CMD_DIRECTION_GET == dir ) {
...
static acamera_cmd_result general_defect_pixel_info( acamera_isp_ctx_ptr_t p_ictx, acamera_cmd_direction dir, const uint32_t *value, uint32_t *ret_value )
{
    // parameter check
    if ( ( CMD_DIRECTION_GET == dir ) && ( NULL == ret_value ) ) {
        LOG( LOG_ERR, "Invalid param: ret_value is NULL." );
...
static acamera_cmd_result general_output_formatter_data( acamera_isp_ctx_ptr_t p_ictx, acamera_cmd_direction dir, const uint32_t *value, uint32_t *ret_value )
{
    // parameter check
    if ( CMD_DIRECTION_SET == dir ) {
        LOG( LOG_NOTICE, "SET is not supported." );
...
static acamera_cmd_result general_mcfe_get_slot_avail_inputs( acamera_isp_ctx_ptr_t p_ictx, acamera_cmd_direction dir, const uint32_t *value, uint32_t *ret_value )
{
    // parameter check
    if ( CMD_DIRECTION_SET == dir ) {
        LOG( LOG_NOTICE, "SET is not supported." );
...
static acamera_cmd_result general_hist_is_on_sqrt( acamera_isp_ctx_ptr_t p_ictx, acamera_cmd_direction dir, const uint32_t *value, uint32_t *ret_value )
{
    // parameter check
    if ( CMD_DIRECTION_SET == dir ) {
        LOG( LOG_NOTICE, "SET is not supported." );
...
// No-operation for PLATFORM_MODEL build configuration
#else
#error "No general router handler for CMD_ID_HIST_IS_ON_SRQT !"
#endif

    return CMD_RESULT_OK;
}

static acamera_cmd_result general_buffer_address_translation( acamera_isp_ctx_ptr_t p_ictx, acamera_cmd_direction dir, const uint32_t *value, uint32_t *ret_value )
{
    // parameter check
    if ( CMD_DIRECTION_GET == dir ) {
        LOG( LOG_NOTICE, "GET is not supported." );
...
#if defined( ISP_HAS_ISP_WRAPPER_FSM ) || defined( ISP_HAS_ISP_WRAPPER_DUMMY_FSM )

    acamera_cmd_buffer_address_translation_info *p_info = (acamera_cmd_buffer_address_translation_info *)value;

    switch ( p_info->raw.type ) {
    case CMD_BUFFER_ADDRESS_TRANSLATION_TYPE_DIRECT:
        isp_wrapper_configure_raw_buffer_addr_translation( p_ictx->context_id, 0, p_info->raw.address.high, p_info->raw.address.low );
        break;
    case CMD_BUFFER_ADDRESS_TRANSLATION_TYPE_TRANSLATE:
        isp_wrapper_configure_raw_buffer_addr_translation( p_ictx->context_id, 1, p_info->raw.address.high, p_info->raw.address.low );
        break;
    default:
        break;
    }

    switch ( p_info->out.type ) {
    case CMD_BUFFER_ADDRESS_TRANSLATION_TYPE_DIRECT:
        isp_wrapper_configure_out_buffer_addr_translation( p_ictx->context_id, 0, p_info->out.address.high, p_info->out.address.low );
        break;
    case CMD_BUFFER_ADDRESS_TRANSLATION_TYPE_TRANSLATE:
        isp_wrapper_configure_out_buffer_addr_translation( p_ictx->context_id, 1, p_info->out.address.high, p_info->out.address.low );
        break;
    default:
        break;
    }
...
// No-operation for PLATFORM_MODEL build configuration
#else
#error "No general router handler for CMD_ID_BUFFER_ADDRESS_TRANSLATION !"
#endif
...
static P_HANDLE handle_arr[CMD_ID_MAX] = {
    [CMD_ID_RAW_SCALER_RATIO] = general_raw_scaler_ratio,
    ...
    [CMD_ID_CMOS_ANALOG_GAIN] = general_cmos_analog_gain,
    [CMD_ID_CMOS_DIGITAL_GAIN] = general_cmos_digital_gain,
    ...
    [CMD_ID_SENSOR_PRESET_INFO] = general_sensor_preset_info,
    ...
    [CMD_ID_BUFFER_ADDRESS_TRANSLATION] = general_buffer_address_translation};

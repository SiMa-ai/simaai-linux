...
#ifdef ACAMERA_FAULTS_CFG_FAULTS_CFG_STATUS_IC_SCONFIG_PCLK2_TOP_FAULT_S_ADDRESS_RANGE_DEFAULT
...
#else
#define acamera_faults_cfg_faults_cfg_status_ic_sconfig_pclk2_top_apb_fault_s_address_range_write( base, val )
#endif /* ACAMERA_FAULTS_CFG_FAULTS_CFG_STATUS_IC_SCONFIG_PCLK2_TOP_FAULT_S_ADDRESS_RANGE_DEFAULT */

#endif /* ACAMERA_FAULTS_CFG_FAULTS_CFG_STATUS_IC_SCONFIG_PCLK2_TOP_APB_FAULT_S_ADDRESS_RANGE_DEFAULT */
...
// Profiler init
    ...
    override_context_param( p_ictx, CMD_INTERFACE_MODE_PARAM, p_ictx->settings.context_options.cmd_if_is_passive_mode );
    ...
    // Set context state parameter
    override_context_param( p_ictx, CONTEXT_STATE_PARAM, CTX_STATE_UNKNOWN );

    acamera_fsmgr_init( &p_ictx->fsmgr );

    // Set context state parameter
    override_context_param( p_ictx, CONTEXT_STATE_PARAM, CTX_STATE_INIT );

    LOG( LOG_INFO, "* End initializing context #%02d ------------", p_ictx->context_id );
    LOG( LOG_INFO, "* -------------------------------------------" );
...
void acamera_isp_ctx_process_interrupt( acamera_isp_ctx_ptr_t p_ictx, uint8_t event )
{
    ...
    if ( event == ACAMERA_IRQ_AE_STATS ) {
        ...
        is_event_stats = 1;
        // Division by 0 is checked
        are_stats_processed = STATS_PROCESSING_RATIO > 0 && ( p_ictx->ae_stats_counter % STATS_PROCESSING_RATIO ) == 0;
    }
    ...
    if ( event == ACAMERA_IRQ_AWB_STATS ) {
        ...
        is_event_stats = 1;
        // Division by 0 is checked
        are_stats_processed = STATS_PROCESSING_RATIO > 0 && ( p_ictx->awb_stats_counter % STATS_PROCESSING_RATIO ) == 0;
    }
    ...
    // Stats IRQ have special logic.
    if ( is_event_stats ) {
        // Block statistics irq when firmware is frozen/on stat skip to block 3a to run.
        // Logic for auto-skipping: only process every STATS_PROCESSING_RATIO-th frame.
        // 1 => all processed ... 4 => processed pattern = 10001000... and so on.
        // (see above where are_stats_processed is assigned to).
        if ( !is_fw_freeze && are_stats_processed ) {
...
int32_t acamera_isp_ctx_process_event( acamera_isp_ctx_ptr_t p_ictx )
{
    ...
    // Process events and handle context state param.
    for ( i = 0; i < ISP_PROCESS_NUM_EVENTS_QUANT; i++ ) {

        const int event_id = acamera_fsmgr_process_event( &( p_ictx->fsmgr ) );

        if ( event_id < 0 ) {
            break;
        }

        // States CTX_STATE_UNKOWN and CTX_STATE_INIT are set in acamera_isp_ctx_init()
        switch ( event_id ) {
        case event_id_fsm_config:
            override_context_param( p_ictx, CONTEXT_STATE_PARAM, CTX_STATE_CONFIG );
            break;
        case event_id_fsm_start:
            override_context_param( p_ictx, CONTEXT_STATE_PARAM, CTX_STATE_START );
            break;
        case event_id_fsm_stop:
            override_context_param( p_ictx, CONTEXT_STATE_PARAM, CTX_STATE_STOP );
            break;
        case event_id_fsm_deinit:
            override_context_param( p_ictx, CONTEXT_STATE_PARAM, CTX_STATE_DEINIT );
            break;
        default:
            break;
        }
...
{
    context_params *params = &( p_ictx->params );
    uint32_t flags, new_value;
    uint8_t ret = SUCCESS;
    struct {
        uint8_t fsm_start : 1;
        uint8_t fsm_stop : 1;
        uint8_t fsm_config : 1;
    } trigger_events = {0};
        ...
        if ( !list_index ) {
            ...
            if ( flags ) {
                if ( flags & PARAM_FLAG_RECONFIG_ON_LIST_VALUE ) {
                    trigger_events.fsm_stop = 1;
                    trigger_events.fsm_config = 1;
                    trigger_events.fsm_start = 1;
                }
            }
        }
    }
    ...
    if ( flags ) {
        ...
        if ( ( flags & PARAM_FLAG_RECONFIG_ON_TRUE_DEFAULT_RESET ) && value ) {
            trigger_events.fsm_stop = 1;
            trigger_events.fsm_config = 1;
            trigger_events.fsm_start = 1;
            new_value = params->params[param_id].default_value;
        } else if ( flags & PARAM_FLAG_REGISTER_REQUEST ) {
            ret = handle_register_set_value( p_ictx, value );
        } else if ( ( flags & PARAM_FLAG_HANDLER_FUNCTION ) && ( params->params[param_id].param_handler != NULL ) ) {
            // Update new value with current parameter value so handler function can
            // decide whether to update parameter value or not
            new_value = params->params[param_id].value;
            ret = params->params[param_id].param_handler( p_ictx, value, COMMAND_SET, &new_value );
        }
        ...
        if ( ( trigger_events.fsm_stop || trigger_events.fsm_config || trigger_events.fsm_start ) &&
             ( get_context_param( p_ictx, CMD_INTERFACE_MODE_PARAM ) == CMD_IF_MODE_ACTIVE ) ) {

            if ( trigger_events.fsm_stop ) {
                raise_context_event( p_ictx, event_id_fsm_stop, EVENT_QUEUE_EVENT_FLAG_FSM_TYPE );
            }

            if ( trigger_events.fsm_config ) {
                raise_context_event( p_ictx, event_id_fsm_config, EVENT_QUEUE_EVENT_FLAG_FSM_TYPE );
            }

            if ( trigger_events.fsm_start ) {
                raise_context_event( p_ictx, event_id_fsm_start, EVENT_QUEUE_EVENT_FLAG_FILTER | EVENT_QUEUE_EVENT_FLAG_FSM_TYPE );
            }
        }
    }
    ...
    if ( ret == SUCCESS ) {
        params->params[param_id].value = new_value;
    }

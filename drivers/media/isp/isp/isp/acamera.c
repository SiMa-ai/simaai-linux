...
static void *get_active_ctx_ptr( void )
{
    return &( g_isp.fw_ctx[get_isp_param( ACTIVE_CONTEXT_PARAM )] );
}

// Helper function to support this module and Command API layer ( only )
void *get_ctx_ptr_by_id( uint32_t ctx_id )
{
    if ( ctx_id >= FIRMWARE_CONTEXT_NUMBER ) {
        LOG( LOG_CRIT, "Invalid parameter. Context id (%u) is out of range [0;%d]",
             ctx_id, ( ( FIRMWARE_CONTEXT_NUMBER > 0 ) ? ( FIRMWARE_CONTEXT_NUMBER - 1 ) : 0 ) );
        return NULL;
    }

    return &( g_isp.fw_ctx[ctx_id] );
}
...
int32_t acamera_init( acamera_settings *settings, uint8_t num_of_contexts, semaphore_t sem )
{
    ...
    LOG( LOG_NOTICE, "DRIVER: " FIRMWARE_VERSION_STR " (%x)", FIRMWARE_REVISION );
    LOG( LOG_NOTICE, "HW GEN: " ISP_RTL_VERSION_STR " (" ISP_RTL_SVN_REVISION_STR ")" );
...
#if 0
        LOG(LOG_INFO,"IRQ slot: %d mask: 0x%x", slot, mask );

		LOG (LOG_ERR, "MCFE FC : %#x", acamera_frontend_mcfe_enable_flow_control_read(p_ictx->settings.isp_base));
		LOG (LOG_ERR, "MCBE FC : %#x", acamera_frontend_mcbe_enable_flow_control_read(p_ictx->settings.isp_base));
		LOG (LOG_ERR, "BE Fifo1 : %#x", acamera_frontend_mcbe_fifo1_watermark_read(p_ictx->settings.isp_base));
		LOG (LOG_ERR, "BE Fifo2 : %#x", acamera_frontend_mcbe_fifo1_watermark_read(p_ictx->settings.isp_base));
		LOG (LOG_ERR, "BE Fifo2 : %#x", acamera_frontend_mcbe_fifo1_watermark_read(p_ictx->settings.isp_base));
		LOG (LOG_ERR, "BE Fifo3 : %#x", acamera_frontend_mcbe_fifo1_watermark_read(p_ictx->settings.isp_base));
#endif
...
static uint8_t set_isp_param_ret( uint32_t param_id, uint32_t value )
{
    ...
    if ( flags ) {
        if ( flags & PARAM_FLAG_RECONFIG ) {
            ...
            const acamera_isp_ctx_ptr_t p_ictx = get_active_ctx_ptr();

            if ( get_context_param( p_ictx, CMD_INTERFACE_MODE_PARAM ) == CMD_IF_MODE_ACTIVE ) {
...
uint8_t acamera_handle_param( uint32_t ctx_id, uint32_t param_id, uint8_t direction, uint32_t set_value, uint32_t *get_value )
{
    if ( param_id & ISP_PARAM_CLASS_MASK ) {
        ...
        return handle_context_param( get_ctx_ptr_by_id( ctx_id ), param_id, direction, set_value, get_value );

...
static int32_t cmos_alloc_sensor_analog_gain( cmos_fsm_ptr_t p_fsm, int32_t gain )
{
    ...
    const int32_t max_gain = MIN( sensor_info.again_log2_max, ( get_context_param( p_ictx, SYSTEM_MAX_SENSOR_ANALOG_GAIN_PARAM ) << ( LOG2_GAIN_SHIFT - 5 ) ) );
    if ( gain > max_gain ) {
        gain = max_gain;
    }
...
static void exposure_set_store_current( cmos_fsm_ptr_t p_fsm, exposure_set_t *p_set )
{
    ...
    general_locked_exp_info_t *locked_exp_info = &sensor_info.current_sensor_mode.channel_info.locked_exp_info;
    ...
    if ( exp_num == 4 ) {
        ...
        if ( locked_exp_info->locked_exp_ratio_short_flag ) {
            p_set->data.exposure_ratio_short = 64 * locked_exp_info->locked_exp_ratio_short_val;
        } else {
            p_set->data.exposure_ratio_short = 64 * (uint32_t)p_fsm->integration_time_medium / p_fsm->integration_time_short;
        }

        if ( locked_exp_info->locked_exp_ratio_medium_flag ) {
            p_set->data.exposure_ratio_medium = 64 * locked_exp_info->locked_exp_ratio_medium_val;
        } else {
            p_set->data.exposure_ratio_medium = 64 * (uint32_t)p_fsm->integration_time_medium2 / p_fsm->integration_time_medium;
        }


        if ( locked_exp_info->locked_exp_ratio_medium2_flag ) {
            p_set->data.exposure_ratio_medium2 = 64 * locked_exp_info->locked_exp_ratio_medium2_val;
        } else {
        ...
        if ( locked_exp_info->locked_exp_ratio_flag ) {
            p_set->data.exposure_ratio = 64 * locked_exp_info->locked_exp_ratio_val;
        } else {
            switch ( sensor_info.isp_exposure_channel_delay ) {
            case 1:
                prev_again = history_get_exposure_set( (cmos_fsm_ptr_t)p_fsm, inttime_delay )->info.again_log2;
...
static void cmos_calculate_multi_exposure( cmos_fsm_ptr_t p_fsm )
{
    ...
    if ( is_multi_exposure( p_ictx, &sensor_info ) ) {
        ...
        if ( ( exp_num == 1 ) && ( is_short_exposure_frame( p_ictx->settings.isp_base ) == 0 ) ) {
            ...
            override_context_param( p_ictx, SYSTEM_MIDDLE2_INTEGRATION_TIME_PARAM, 0 );
            ...
            override_context_param( p_ictx, SYSTEM_MIDDLE2_INTEGRATION_TIME_PARAM, exp_set.integration_time_medium2 );
...
static void cmos_write_isp_settings( cmos_fsm_const_ptr_t p_fsm )
{
    ...
    // If history is not yet populated (likely to happen on the first frame), skip registers update
    if ( !history_is_valid( p_fsm ) ) {
        return;
    }

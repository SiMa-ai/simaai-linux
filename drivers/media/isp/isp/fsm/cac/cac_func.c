...
#ifdef ISP_HAS_RAW_SCALER_FSM
static inline uint64_t bit_shift64( uint64_t v, int32_t s )
{
    return ( s > 0 ) ? ( v << s ) : ( v >> ( -s ) );
}

static int64_t signed_bitshift64( int64_t val, int32_t shift )
{
    int64_t out_val = 0;
    uint8_t val_sign = val < 0;
    val = ACAMERA_ABS( val );
...
static void cac_lut_reload( cac_fsm_ptr_t p_fsm )
{
    ...
    /* MOSS */
    if ( sensor_info.active_width / calibration_ca_mesh_width < 20 ) {
#else
    /* MORGAN */
    ...
    // ca_correction_mem_array size in 32-bit registers
    const uint32_t mem_size_regs = ACAMERA_CA_CORRECTION_MEM_SIZE / ( ACAMERA_CA_CORRECTION_MEM_ARRAY_DATA_DATASIZE >> 3 );
    uint32_t index;

    // Reset ca correction memory to default values
    for ( index = 0; index < mem_size_regs; index++ ) {
        acamera_ca_correction_mem_array_data_write( isp_base, index, ACAMERA_CA_CORRECTION_MEM_ARRAY_DATA_DEFAULT );
    }
    ...
    uint16_t component;
#ifdef ISP_HAS_RAW_SCALER_FSM
    uint32_t raw_scaler_invert_ratio;
    if ( WRAP_GENERAL_CMD( ACAMERA_FSM2ICTX_PTR( p_fsm ),
                           CMD_ID_RAW_SCALER_RATIO,
                           CMD_DIRECTION_GET,
                           NULL,
                           &raw_scaler_invert_ratio ) ) {
        LOG( LOG_ERR, "CMD_ID_RAW_SCALER_RATIO command failed" );
        return;
    }
#endif
...
#ifdef ISP_HAS_RAW_SCALER_FSM
            ca_model[term] = (int32_t)signed_bitshift64( ( (int64_t)ca_model[term] * raw_scaler_invert_ratio ), -20 );
#endif /* ISP_HAS_RAW_SCALER_FSM */
        }

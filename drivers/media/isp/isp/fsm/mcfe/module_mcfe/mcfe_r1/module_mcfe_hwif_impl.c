...
/**
 * @brief      Gets buffer line offset.
 *
 * @param[in]  bftype        Buffer type
 * @param[in]  idx           Buffer index
 *
 */
uint32_t mcfe_hwif_buff_line_offset_get( const mcfe_hwif_buf_type_t bftype, const uint8_t idx )
{
    ISP_BUFFER_T *ptr = mcfe_hwif_buff_get_ptr( bftype, idx );

    if ( ptr ) {
        return ( acamera_buffers_line_offset_read( ptr ) );
    } else {
        LOG( LOG_ERR, "No valid buffer pointer." );
        return 0;
    }
 ...
 *
 */
uint32_t mcfe_hwif_buff_crc_get( mcfe_hwif_buf_type_t bftype, const uint8_t idx )
{
    ISP_BUFFER_T *ptr = mcfe_hwif_buff_get_ptr( bftype, idx );

    if ( ptr ) {
        return acamera_buffers_buffer_crc_read( ptr );
    } else {
        LOG( LOG_ERR, "No valid buffer pointer." );
        return 0;
    }

...
static int bufset_validate_frames( aframe_t *frames[], const size_t num_frames, const unsigned int flags )
{
    size_t i, j;
    ...
    for ( i = 0; i < num_frames; i++ ) {
        ...
        for ( j = 0; j < frames[i]->num_planes; j++ ) {
            ...
            if ( ( frames[i]->planes[j].address.low == 0 ) && ( ( flags & MCFE_BUFSET_FLAG_CREATE_EMPTY ) == 0 ) ) {
                LOG( LOG_ERR, "Incorrect plane address %d (frame %lu, plane %lu ).",
                     frames[i]->planes[j].address.low, i, j );
                return MCFE_ERR_INV_PARM;
            }
        }
    }
...
static void bufset_config_header( module_mcfe_bufset_t *bufset, aframe_t *frames[], module_mcfe_bufset_type_t type, mcfe_hwif_buf_type_t buffer_type, unsigned int num_frames, const unsigned int flags )
{
    size_t i;
    ...
    if ( ( flags & MCFE_BUFSET_FLAG_CREATE_EMPTY ) == 0 ) {
        for ( i = 0; i < num_frames; i++ ) {
            bufset->frames.frame[i] = frames[i];
        }
    }

    for ( i = 0; i < frames[0]->num_planes; i++ ) {
        bufset->frames.cached_planes[i].data_width = frames[0]->planes[i].data_width;
        bufset->frames.cached_planes[i].height = frames[0]->planes[i].height;
        bufset->frames.cached_planes[i].width = frames[0]->planes[i].width;
        bufset->frames.cached_planes[i].line_offset = frames[0]->planes[i].line_offset;
        bufset->frames.cached_planes[i].hw_cfg = frames[0]->planes[i].hw_cfg;
    }

    bufset->type = type;
    bufset->frames.num_frames = num_frames;
    bufset->frames.num_planes = frames[0]->num_planes;
...
static int bufset_create_buffers( module_mcfe_bufset_t *bufset, aframe_t *frames[] )
{
    size_t i, j;
    ...
    for ( i = 0; i < bufset->frames.num_frames; i++ ) {
        for ( j = 0; j < bufset->buffs.num_planes; j++ ) {
            ...
            if ( bufset->flags & MCFE_BUFSET_FLAG_CREATE_EMPTY ) {
                buffer_config.address = 0;
            } else {
                buffer_config.address = plane->address.low;
            }
            ...
            LOG( LOG_INFO, "Buffer set buffer created:\n"
                            "\tenabled: %u\n"
                            "\tindex: %u\n"
                            "\ttype: %u\n"
                            "\twidth: %u\n"
...
static int bufset_get_frame_index_with_status( const module_mcfe_bufset_t *bufset, const int frame_no, const module_mcfe_bufset_status_t status )
{
    ...
    case MCFE_BUFSET_STATUS_FILLED:
        req_buffer_status = 1;
        break;
    case MCFE_BUFSET_STATUS_ANY:
        req_buffer_status = 255;
        break;
    default:
...
static int bufset_set_buffer_address_and_status( const module_mcfe_bufset_t *bufset, const aframe_t *frame, const int frame_index, const module_mcfe_bufset_status_t status, bool is_input_frame_supplied )
{
    int buffer_status;
    size_t i;

    assert( bufset );
    assert( frame || !is_input_frame_supplied );
...
static int bufset_get_buffer_crc( const module_mcfe_bufset_t *bufset, aframe_t *frame, const int frame_index )
{
    size_t i;
    ...
    for ( i = 0; i < bufset->buffs.num_planes; i++ ) {
        module_mcfe_buf_t *buffer = bufset->buffs.buff[frame_index][i];
        frame->planes[i].crc = mcfe_hwif_buff_crc_get( buffer->type, buffer->index ) & 0xFFFF; // 32-bit register (Frame counter | CRC-16)
    }
...
int module_mcfe_bufset_create( aframe_t *frames[], module_mcfe_bufset_t **bufset, const unsigned int num_frames, const unsigned int flags )
{
    ...
    if ( bufset_validate_frames( frames, num_frames, flags ) != MCFE_ERR_NONE ) {
    ...
    bufset_config_header( lbufset, frames, bufset_type, buffer_type, num_frames, flags );

    if ( bufset_create_buffers( lbufset, frames ) != MCFE_ERR_NONE ) {
                             ...
                             const module_mcfe_bufset_swap_mode_t mode )
{
    ...
    bool is_input_frame_supplied = ( ( mode != MCFE_BUFSET_SWAP_NO_IN_EMPTY_OUT ) &&
                                     ( mode != MCFE_BUFSET_SWAP_NO_IN_FILLED_OUT ) &&
                                     ( mode != MCFE_BUFSET_SWAP_NO_IN_ANY_OUT ) );

    // Swap operation expects output frame with any status, there might be no ( likely ) output frames at all (empty buffset)
    bool is_output_frame_expected = ( ( mode != MCFE_BUFSET_SWAP_EMPTY_IN_ANY_OUT ) &&
                                      ( mode != MCFE_BUFSET_SWAP_FILLED_IN_ANY_OUT ) );
    ...
    if ( ( in == NULL ) && is_input_frame_supplied ) {
    ...
    case MCFE_BUFSET_SWAP_FILLED_IN_EMPTY_OUT:
        ...
        break;
    case MCFE_BUFSET_SWAP_EMPTY_IN_EMPTY_OUT:
        get_status = MCFE_BUFSET_STATUS_EMPTY;
        set_status = MCFE_BUFSET_STATUS_EMPTY;
        get_status_desc = "empty";
        set_status_desc = "empty";
        break;
    case MCFE_BUFSET_SWAP_FILLED_IN_ANY_OUT:
        get_status = MCFE_BUFSET_STATUS_ANY;
        set_status = MCFE_BUFSET_STATUS_FILLED;
        get_status_desc = "any";
        set_status_desc = "filled";
        break;
    case MCFE_BUFSET_SWAP_EMPTY_IN_ANY_OUT:
        get_status = MCFE_BUFSET_STATUS_ANY;
        set_status = MCFE_BUFSET_STATUS_EMPTY;
        get_status_desc = "any";
        set_status_desc = "empty";
        break;
    case MCFE_BUFSET_SWAP_NO_IN_EMPTY_OUT:
        get_status = MCFE_BUFSET_STATUS_EMPTY;
        set_status = MCFE_BUFSET_STATUS_EMPTY;
        get_status_desc = "empty";
        set_status_desc = "empty";
        break;
    case MCFE_BUFSET_SWAP_NO_IN_FILLED_OUT:
        get_status = MCFE_BUFSET_STATUS_FILLED;
        set_status = MCFE_BUFSET_STATUS_EMPTY;
        get_status_desc = "filled";
        set_status_desc = "empty";
        break;
    case MCFE_BUFSET_SWAP_NO_IN_ANY_OUT:
        get_status = MCFE_BUFSET_STATUS_ANY;
        set_status = MCFE_BUFSET_STATUS_EMPTY;
        get_status_desc = "any";
        set_status_desc = "empty";
    ...
    if ( bufset_set_buffer_address_and_status( bufset, in, frame_index, set_status, is_input_frame_supplied ) != MCFE_ERR_NONE ) {
    ...
    *out = bufset->frames.frame[frame_index];
    if ( is_input_frame_supplied ) {
        ...
        bufset->frames.frame[frame_index] = NULL;
    }

    // Read out buffer CRCs, check if we expect output buffer to be a valid frame pointer
    if ( ( *out != NULL ) || is_output_frame_expected ) {
        if ( bufset_get_buffer_crc( bufset, *out, frame_index ) != MCFE_ERR_NONE ) {
            LOG( LOG_ERR, "Failed to get buffer crc of the buffer set (type = %d).", bufset->type );
            return MCFE_ERR_GENERAL;
        }
    }

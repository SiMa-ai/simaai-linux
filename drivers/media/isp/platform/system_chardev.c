...
static int isp_fops_open( struct inode *inode, struct file *f )
{
    ...
    if ( !isp_dev_ctx.dev_inited ) {
        LOG( LOG_ERR, "Dev is not initialized, failed to open." );
        return -ERESTARTSYS;
    }
    ...
    if ( !p_ctx ) {
        LOG( LOG_CRIT, "Fatal error, isp dev contexts are wrong, contents dump:" );
...
static int isp_fops_release( struct inode *inode, struct file *f )
{
    ...
    if ( p_ctx != &isp_dev_ctx ) {
        LOG( LOG_ERR, "Invalid parameter: %p.", p_ctx );
        return -EINVAL;
    }
...
static ssize_t isp_fops_write( struct file *file, const char __user *buf, size_t count, loff_t *ppos )
{
    ...
    /* isp_dev_uf device only supports read at the moment */
    if ( p_ctx != &isp_dev_ctx ) {
        LOG( LOG_ERR, "Invalid parameter: %p.", p_ctx );
        return -EINVAL;
    }
...
int system_chardev_init( void )
{
    ...
    p_ctx0 = &isp_dev_ctx;
    system_memset( p_ctx0, 0, sizeof( *p_ctx0 ) );
...
int system_chardev_read( char *data, int size )
{
    ...
    if ( !isp_dev_ctx.dev_inited ) {
        LOG( LOG_ERR, "Dev is not initialized, failed to read." );
        return -1;
    }
...
int system_chardev_write( const char *data, int size )
{
    ...
    LOG( LOG_DEBUG, "Wake up reader waiting on kfifo out." );
...
int system_chardev_destroy( void )
{
    if ( isp_dev_ctx.dev_inited ) {
        ...
        LOG( LOG_DEBUG, "misc_deregister dev: %s.", isp_dev_ctx.dev_name );
    } else {

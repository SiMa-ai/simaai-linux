...
//int32_t init_semaphore(semaphore_t *sem)
int32_t system_semaphore_init( semaphore_t *sem, uint32_t init_val )
{
    struct semaphore *sys_sem = kmalloc( sizeof( struct semaphore ), GFP_KERNEL | __GFP_NOFAIL );
    *sem = sys_sem;
    sema_init( sys_sem, init_val );
    return 0;
}

//int32_t raise_semaphore(semaphore_t sem)
...
//int32_t wait_semaphore(semaphore_t sem, uint32_t timeout_ms)

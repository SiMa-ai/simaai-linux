...
static void sensor_deinit_imx568( void *sensor_priv )
{
    sensor_private_t *priv = sensor_priv;
...
void sensor_init_imx568( void **priv_ptr, uint8_t location, sensor_control_t *ctrl, const sensor_options_t *const options )
{
    LOG(LOG_DEBUG, "imx568 sensor init for ctx : %u", location);
    ...
    ctrl->deinit = sensor_deinit_imx568;

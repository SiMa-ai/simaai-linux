...
#include "kobe1_FPGA_I2C_to_SPI_algo.h"
#include "kobe1_FPGA_I2C_to_SPI_data.h"

#include "econ_kobe1_fpga_fw.txt"
#include "econ_kobe1_fpga_bootloader.h"
 ...
 *                                                            *
 *************************************************************/
static int ReadBytesAndSendNACK(struct i2c_client *client, int count, unsigned char *val, int NAck)
{
	int ret, i;
...
#ifdef I2C_DEBUG
		pr_info("%s msg_count = %d count = %d flags = %d bus = %d\n",
				__func__, msg_count, msg[msg_count].len, msg[msg_count].flags,
				client->adapter->nr);
#endif
		msg_count++;
		ret = i2c_transfer(client->adapter, msg, msg_count);
...
static int SendBytesAndCheckACK(struct i2c_client *client, int count, unsigned char *val)
{
	int i;
	...
	if (restart || (msg_count == 0)) {
		...
		if (!buf[msg_count]) {
#ifdef I2C_DEBUG
			pr_err("Memory alocation failed\n");
#endif
			return -ENOMEM;
		}
		for (i = 0; i < (count/8); i++)
...
#ifdef I2C_DEBUG
		pr_info("%s msg_count = %d count = %d flags = %d bus = %d\n",
				__func__, msg_count, msg[msg_count-1].len,
				msg[msg_count-1].flags, client->adapter->nr);
#endif
...
void SetI2CDelay(unsigned int a_msTimeDelay)
{
	...
	for (ms_index = 0; ms_index < a_msTimeDelay; ms_index++) {
		...
		/*each loop should delay for 1 microsecond or more.*/
			loop_index = 0;
			do {
				/*The NOP fakes the optimizer out so that
				 * it doesn't toss out the loop code entirely
				 */
				//__asm NOP
			} while (loop_index++ < ((g_usCpu_Frequency/8) +
						((g_usCpu_Frequency % 8) ? 1 : 0)));
			/*use do loop to force at least one loop*/
		}
	}
}
/*************************************************************
...
static short int ispVMRead(struct i2c_client *client, unsigned int a_uiDataSize)
{
	...
	InData = (unsigned char *) kmalloc((a_uiDataSize+7)/8+1, GFP_KERNEL);
	if (InData == NULL)
		return ERR_OUT_OF_MEMORY;

	ReadData = (unsigned char *) kmalloc((a_uiDataSize+7)/8+1, GFP_KERNEL);
	if (ReadData == NULL)
		return ERR_OUT_OF_MEMORY;
	...
	for (uiIndex = 0; uiIndex < a_uiDataSize; uiIndex++) {
		if (uiIndex % 8 == 0) {
			if (g_usDataType & TDI_DATA) {
				 ...
				 *                                                            *
				 *************************************************************/
					if ((g_usDataType & COMPRESS_FRAME) &&
							(ucTDIByte == (unsigned char) 0xFF)) {
						g_ucCompressCounter =
							GetByte(g_iMovingDataIndex++, 0);
						g_ucCompressCounter--;
					}
				}
			} else {
	...
	for (uiIndex = 0; uiIndex < a_uiDataSize; uiIndex++) {
...
#ifdef I2C_DEBUG
				pr_info("%.2X g_iTDOIndex=%d", ucTDOByte, g_iTDOIndex);
#endif
			} else {
				/*************************************************************
				 *                                                            *
...
#ifdef I2C_DEBUG
					pr_info("%.2X g_iMovingDataIndex=%d",
							ucTDOByte, g_iMovingDataIndex);
#endif
		...
		if ((((ucMaskByte << uiIndex % 8) & 0x80) ? 0x01 : 0x00)) {
			if (ucCurBit != (unsigned char) (((ucTDOByte << uiIndex % 8) & 0x80) ?
						0x01 : 0x00))
				usErrorCount++;
		}
	}
	...
	for (usDataSizeIndex = 0; usDataSizeIndex < (unsigned short)((a_uiDataSize + 7) / 8);
			usDataSizeIndex++) {
		cInDataByte = ReadData[usDataSizeIndex];
		pr_info("%.2X usDataSizeIndex=%d", cInDataByte, usDataSizeIndex);
...
static short int ispVMShiftExec(unsigned int a_uiDataSize)
{
	...
	g_usDataType &= ~(TDI_DATA + TDO_DATA + MASK_DATA + DTDI_DATA +
			DTDO_DATA + COMPRESS_FRAME);
...
static short int ispVMSend(struct i2c_client *client, unsigned int a_uiDataSize)
{
	...
	g_pucInData = (unsigned char *) kmalloc((a_uiDataSize+7)/8+1, GFP_KERNEL);
	if (g_pucInData == NULL)
		return -1;
	...
	if (SendBytesAndCheckACK(client, a_uiDataSize, g_pucInData)) {
		pr_info("Failed to get ACK when send byte.\n");
		kfree(g_pucInData);
		g_pucInData = NULL;
	...
	for (usDataSizeIndex = (unsigned short)((a_uiDataSize + 7) / 8);
...
int write_spi_data_to_fpga(struct i2c_client *client)
{
	...
	retcode = fpga_write_reg(client, FPGA_SLAVE_ADDR, 0x0005,
			(array_traverse_length >> 8) & 0xff);
	if (retcode) {
		dev_err(&client->dev, "Unable to disable I2C Bypass %s(%d)\n",
				__func__, __LINE__);
		return retcode;
	...
	retcode = fpga_write_reg(client, FPGA_SLAVE_ADDR, 0x0006,
			(array_traverse_length >> 16) & 0xff);
	if (retcode) {
		dev_err(&client->dev, "Unable to disable I2C Bypass %s(%d)\n",
				__func__, __LINE__);
		return retcode;
	...
	do {
		...
		if (retcode) {
			dev_err(&client->dev, "Unable to read data from FPGA %s(%d)\n",
					__func__, __LINE__);
			return retcode;
		}
		mdelay(5);
	...
	dev_info(&client->dev, "SPI Flash Write is completed successfully.\n");
...
short int fpga_init(struct i2c_client *client)
{
	...
	for (cVersionIndex = 0; g_szSupportedVersions[cVersionIndex] != 0; cVersionIndex++) {
		for (iIndex = 0; iIndex < strlen(g_szSupportedVersions[cVersionIndex]);
				iIndex++) {
			if (szFileVersion[iIndex] !=
					g_szSupportedVersions[cVersionIndex][iIndex]) {

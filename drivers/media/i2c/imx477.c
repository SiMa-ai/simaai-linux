...
/* Data lane mode */
#define IMX477_DATA_LANE_MODE   CCI_REG8(0x0114)
...
/* 1080p 32fps binned and cropped mode */
static const struct cci_reg_sequence mode_2048x1080_regs[] = {
	{CCI_REG8(0x0344), 0x00},//X_ADD_STA[12:8]
	{CCI_REG8(0x0345), 0x00},//X_ADD_STA[7:0]
	{CCI_REG8(0x0346), 0x01},//Y_ADD_STA[12:8]
	{CCI_REG8(0x0347), 0xb8},//Y_ADD_STA[7:0]
	{CCI_REG8(0x0348), 0x0f},//X_ADD_END[12:8]
	{CCI_REG8(0x0349), 0xd7},//X_ADD_END[7:0]
	{CCI_REG8(0x034a), 0x0a},//Y_ADD_END[12:8]
	{CCI_REG8(0x034b), 0x27},//Y_ADD_END[7:0]

	{CCI_REG8(0x0900), 0x01},
	{CCI_REG8(0x0901), 0x22}, //In RAW12 mode H2 is ignored
	{CCI_REG8(0x3c01), 0x03},
	{CCI_REG8(0x3c02), 0xa2},
	{CCI_REG8(0x5748), 0x07},
	{CCI_REG8(0x5749), 0xff},
	...
	/* Adjust DIG_CROP values to center the crop
	 * or crop from top left/right...etc
	 */
	{CCI_REG8(0x0408), 0x00},//DIG_CROP_X_OFFSET[12:8]
	{CCI_REG8(0x0409), 0x00},//DIG_CROP_X_OFFSET[7:0]
	{CCI_REG8(0x040a), 0x00},//DIG_CROP_Y_OFFSET[12:8]
	{CCI_REG8(0x040b), 0x00},//DIG_CROP_Y_OFFSET[7:0]

	/* In RAW12 mode we cant do H2 binning. So from binning,
	 * we have full width. Retain the same full width for
	 * next stage.
	 */
	{CCI_REG8(0x040c), 0x0f},//DIG_CROP_IMAGE_WIDTH[12:8]
	{CCI_REG8(0x040d), 0xd8},//DIG_CROP_IMAGE_WIDTH[7:0]
	{CCI_REG8(0x040e), 0x04},//DIG_CROP_IMAGE_HEIGHT[12:8]
	{CCI_REG8(0x040f), 0x38},//DIG_CROP_IMAGE_HEIGHT[7:0]

	/* X_OUT_SIZE and Y_OUT_SIZE determines final image
	 * dimensions. This value should match the .width and
	 * .height values in supported modes.
	 */
	{CCI_REG8(0x034c), 0x08},//X_OUT_SIZE[12:8]
	{CCI_REG8(0x034d), 0x00},//X_OUT_SIZE[7:0]
	{CCI_REG8(0x034e), 0x04},//Y_OUT_SIZE[12:8]
	{CCI_REG8(0x034f), 0x38},//Y_OUT_SIZE[7:0]
...
/* 1080p 34fps binned and cropped mode */
static const struct cci_reg_sequence mode_1920x1080_regs[] = {
	 ...
	 * .height values in supported modes.
	 */
	{CCI_REG8(0x034c), 0x07},//X_OUT_SIZE[12:8]
	{CCI_REG8(0x034d), 0x80},//X_OUT_SIZE[7:0]
	{CCI_REG8(0x034e), 0x04},//Y_OUT_SIZE[12:8]
	{CCI_REG8(0x034f), 0x38},//Y_OUT_SIZE[7:0]
...
static const struct imx477_mode supported_modes[] = {
	...
	{
		/* 1080p 32fps binned cropped mode */
		.width = 2048,
		.height = 1080,
		.crop = {
			.left = IMX477_PIXEL_ARRAY_LEFT,
		...
		.frm_length_default = 2197,
		.reg_list = {
			.num_of_regs = ARRAY_SIZE(mode_2048x1080_regs),
			.regs = mode_2048x1080_regs,
		},
	},
	{
...
struct imx477 {
	...
	/* Number CSI data lanes read from device tree */
...
static u8 imx477_get_dt(u32 code)
{
	switch (code) {
	default:
	case MEDIA_BUS_FMT_SRGGB12_1X12:
		...
		struct v4l2_mbus_frame_desc *fd)
{
	...
	code = v4l2_subdev_state_get_format(state, IMAGE_PAD)->code;
	...
	fd->entry->bus.csi2.dt = imx477_get_dt(code);

	return 0;
}

static int imx477_set_format(struct v4l2_subdev *sd,
		struct v4l2_subdev_state *state,
                             ...
                             struct v4l2_subdev_state *state)
{
        struct v4l2_subdev_format format = {
                .format = {
                        .width = IMX477_NATIVE_WIDTH,
                        .height = IMX477_NATIVE_HEIGHT,
                },
        };

        imx477_set_format(sd, state, &format);
...
static int imx477_start_streaming(struct imx477 *imx477)
{
	...
	if (!imx477->common_regs_written) {
		...
		cci_write(imx477->regmap, IMX477_DATA_LANE_MODE,
				(imx477->num_data_lanes - 1), &ret);
		if (ret) {
			dev_err(&client->dev, "Failed to write reg 0x%4.4x. error = %d\n",
					IMX477_DATA_LANE_MODE, ret);
		}
...
static int imx477_check_hwcfg(struct device *dev, struct imx477 *imx477)
{
	...
	if ((ep_cfg.bus.mipi_csi2.num_data_lanes) == 0 || (ep_cfg.bus.mipi_csi2.num_data_lanes) > 4) {
		dev_err(dev, "invalid lanes configured\n");
		goto error_out;
	}
	imx477->num_data_lanes = ep_cfg.bus.mipi_csi2.num_data_lanes;
...
static int imx477_probe(struct i2c_client *client)
{
	...
	if (ret) {
		dev_dbg(dev, "Regulators not found.\n");
	}

...
/* Data lane mode */
#define IMX477_DATA_LANE_MODE	0x0114
...
static const struct imx477_reg mode_4032x3040_regs[] = {
	{0x0342, 0x60},
	{0x0343, 0x89},
	{0x0344, 0x00},
	{0x0345, 0x00},
...
static const struct imx477_reg mode_1920x1080_regs[] = {
	...
	{0x0901, 0x22}, //binnning type
	{0x0902, 0x02}, //binning weight
	{0x3140, 0x02}, //ND
	{0x3c00, 0x00}, //
	{0x3c01, 0x03},
	{0x3c02, 0xa2},
	{0x3f0d, 0x01}, //ADbit mode
	{0x5748, 0x07},
	{0x5749, 0xff},
	{0x574a, 0x00},
	{0x574b, 0x00},
	...
	{0x0404, 0x00}, //Scale factor
	{0x0405, 0x20}, //scale m
	{0x0408, 0x00},
	{0x0409, 0x00},
	{0x040a, 0x00},
	{0x040b, 0x00},
	{0x040c, 0x0f}, //image width after cropping
	{0x040d, 0xd8},
	{0x040e, 0x04}, //height after digita cropping
	{0x040f, 0x38},
	{0x034c, 0x07}, //X_OUT_SIZE
	{0x034d, 0x80},
	{0x034e, 0x04}, // Y_OUT_SIZE
	{0x034f, 0x38},
	{0x0301, 0x05}, //PIXEL_CLK_DIV
	...
	{0x0823, 0x00},
	{0x080a, 0x00}, //TCLK_POST_EX[9:8]
	{0x080b, 0x7f},
	{0x080c, 0x00}, //THS_PREPARE_EX[9:8]
	{0x080d, 0x4f},
	{0x080e, 0x00}, //THS_ZERO_MIN_EX[9:8]
	{0x080f, 0x77},
	{0x0810, 0x00}, //THS_TRAIL_EX[9:8]
	{0x0811, 0x5f},
	{0x0812, 0x00}, //TCLK_TRAIL_MIN_EX[9:8]
	{0x0813, 0x57},
	{0x0814, 0x00}, //TCLK_PREPARE_EX[9:8]
	{0x0815, 0x4f},
	{0x0816, 0x01}, //TCLK_ZERO_EX[9:8]
	{0x0817, 0x27},
	{0x0818, 0x00}, //TLPX_EX[9:8]
	{0x0819, 0x3f},
	{0xe04c, 0x00},
	{0xe04d, 0x7f},
	{0xe04e, 0x00},
	...
	{0x3f56, 0x01}, //LINE_LENGTH_INCK
	{0x3f57, 0xac},
};

/* 1080p cropped mode */
static const struct imx477_reg mode_2048x1080_regs[] = {
	{0x0342, 0x30},
	{0x0343, 0x65},
	{0x0344, 0x00},
	{0x0345, 0x00},
...
/*  2048x1080 RAW10 */
static const struct imx477_reg mode_10bit_2048x1080_regs[] = {
	{0x420b, 0x01},
	{0x990c, 0x00},
	{0x990d, 0x08},
	{0x9956, 0x8c},
...
/* 3968x3040 RAW10 27fps */
static const struct imx477_reg mode_3968x3040_regs[] = {
	{0x420b, 0x01},
	{0x990c, 0x00},
	{0x990d, 0x08},
	{0x9956, 0x8c},
...
static const struct imx477_mode supported_modes_12bit[] = {
	...
	{
		/* 1080p 62fps cropped mode */
		.width = 2048,
		.height = 1080,
		.line_length_pix = 0x3065,
		.crop = {
			.left = IMX477_PIXEL_ARRAY_LEFT,
			.top = IMX477_PIXEL_ARRAY_TOP + 440,
			.width = 4096,
...
static const struct imx477_mode supported_modes_10bit[] = {
	...
	},
	{
		...
		.width = 2048,
		.height = 1080,
		.line_length_pix = 0x28ca,
		.crop = {
			.left = IMX477_PIXEL_ARRAY_LEFT + 308,
			.top = IMX477_PIXEL_ARRAY_TOP + 450,
			.width = 2664,
			.height = 1980,
		},
		.timeperframe_min = {
		...
		.reg_list = {
			.num_of_regs = ARRAY_SIZE(mode_10bit_2048x1080_regs),
			.regs = mode_10bit_2048x1080_regs,
		}
	},
	{
		/* 27fps */
		...
		.line_length_pix = 0x28ca,
		.crop = {
			.left = IMX477_PIXEL_ARRAY_LEFT + 44,
			.top = IMX477_PIXEL_ARRAY_TOP,
			.width = 4056,
			.height = 3040,
		},
		...
		.reg_list = {
			.num_of_regs = ARRAY_SIZE(mode_3968x3040_regs),
			.regs = mode_3968x3040_regs,
		}
	}
};
...
static int imx477_start_streaming(struct imx477 *imx477)
{
	...
	if (!imx477->common_regs_written) {
		...
		ret = imx477_write_reg(imx477, IMX477_DATA_LANE_MODE, 1, (imx477->num_data_lanes - 1));
		if (ret) {
...
static int imx477_check_hwcfg(struct device *dev)
{
	...
	struct i2c_client *i2c_client = container_of(dev, struct i2c_client, dev);
	struct imx477 *imx477 = to_imx477(i2c_get_clientdata(i2c_client));
	...
	if ((ep_cfg.bus.mipi_csi2.num_data_lanes) == 0 || (ep_cfg.bus.mipi_csi2.num_data_lanes) > 4) {
		dev_err(dev, "configured invalid data lanes, %d\n");
		goto error_out;
	}

	dev_dbg(dev, "Data lanes are %d\n", ep_cfg.bus.mipi_csi2.num_data_lanes);

				      ...
				      phy_interface_t interface)
{
	...
	} else if (xpcs->info.is_simaai_soc) {
		ret = simaai_xpcs_switch_mode(xpcs, interface);
...
static int xpcs_config_aneg_c37_usxgmii(struct dw_xpcs *xpcs,
					unsigned int mode)
{
	int mdio_ctrl, ret, mask, val = 0, tx_conf;
	...
	if (xpcs->info.is_simaai_soc) {
		ret = xpcs_read_vpcs(xpcs, DW_VR_XS_PCS_DIG_CTRL1);
		xpcs_write_vpcs(xpcs, DW_VR_XS_PCS_DIG_CTRL1, DW_CL37_BP | ret);
	}

	mask = DW_VR_MII_PCS_MODE_MASK | DW_VR_MII_TX_CONFIG_MASK;

	if (xpcs->info.is_simaai_soc) {
		val |= DW_VR_MII_C37_ANSGM_SP_LNKSTS;
		val |= DW_VR_MII_AN_INTR_EN;
	}

	if ((xpcs->info.pma == WX_TXGBE_XPCS_PMA_10G_ID) || (xpcs->info.is_simaai_soc)) {
	...
	ret = xpcs_write(xpcs, MDIO_MMD_VEND2, MII_BMCR, mdio_ctrl);
	if (ret < 0)
		return ret;
				      ...
				      unsigned int neg_mode)
{
	...
	if (xpcs->info.is_simaai_soc) {
		mdio_ctrl |= SGMII_SPEED_SS6;
		mdio_ctrl &= ~SGMII_SPEED_SS13;
		xpcs_write(xpcs, MDIO_MMD_VEND2, MII_BMCR, mdio_ctrl);
		ret = xpcs_read(xpcs, MDIO_MMD_VEND2, MII_ADVERTISE);
		ret |= ADVERTISE_1000XFULL;
		xpcs_write(xpcs, MDIO_MMD_VEND2, MII_ADVERTISE, ret);
	}
				    ...
				    struct phylink_link_state *state)
{
	...
	if (ret & DW_VR_MII_AN_INTR_EN) {
		u32 val;
		ret &= ~(DW_VR_MII_AN_INTR_EN);
		ret = xpcs_write(xpcs, MDIO_MMD_VEND2, DW_VR_MII_AN_INTR_STS, ret);
		if (ret < 0)
			return ret;
					 ...
					 int speed, int duplex)
{
	...
	if (neg_mode == PHYLINK_PCS_NEG_INBAND_ENABLED && !xpcs->info.is_simaai_soc)
		return;

...
static int xpcs_plat_init_res(struct dw_xpcs_plat *pxpcs)
{
	...
	struct dw_xpcs_info *info = device_get_match_data(dev);
	...
	info->skip_reset = device_property_present(dev, "snps,skip-reset");
	info->scratch = 0;
	info->is_simaai_soc = device_is_compatible(dev, "simaai,dw-xpcs");

...
static void dw_dphy_ignore_deskew(struct dw_dphy_rx *dphy)
{
	/* Write h1 to the equalizer registers */

	//Write equalizer registers for lane 0
	dw_dphy_te_write(dphy, 0x509, 0x01);

	//Write equalizer registers for lane 1
	dw_dphy_te_write(dphy, 0x709, 0x01);

	//Write equalizer registers for lane 2
	dw_dphy_te_write(dphy, 0x909, 0x01);

	//Write equalizer registers for lane 3
	dw_dphy_te_write(dphy, 0xb09, 0x01);
}
